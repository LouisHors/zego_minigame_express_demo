...
extern "C" {
	...
	//socket tcp
	_ZEGO_API boolean	zegosocket_connect(zegosocket s,const utf8* addr,uint16 port);
...
_ZEGO_API int32	zegosocket_v6_sendto(zegosocket s, const void* data, uint32 len, const utf8* ip, int ip_type, uint16 port);
