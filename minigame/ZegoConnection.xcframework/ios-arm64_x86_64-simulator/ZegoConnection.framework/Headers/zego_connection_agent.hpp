    ...
    public:
        ...
        */
        ...
        /**
         ...
         指定 wss 时位sni host
         */
        void SetNetAgentDefaultDomain(const std::vector<std::pair<std::string, std::string>>& domain_sni_list);
        ...
        @param domain 默认统一接入调度域名
        */
        void SetNetAgentBackupInfo(const std::vector<std::string>& ips);

        /**
        设置统一接入调度的anycastip

         @param ips ip 列表
         @param supportList 支持的端口列表
          @attention 当本地域名解析超时时，会通过此方式来进行调度 获取统一接入链接列表
        */
        void SetNetAgentAnyCastIpInfo(const std::vector<std::string>& ips, const std::vector<int> supportList);
       ...
       */
        void SetNetAgentRootCertificate(std::shared_ptr<CZegoCACertData> cert_data);
         ...
         /**
         设置用于统一接入 quic校验服务端的根证书 quic 都使用该证书
    
         @param content 根证书内容
         @attention  SetNetAgentConfig 中 开启 enableClientVerifyServerCert = true  优先使用ECCRoot 失败时使用 SetNetAgentRootCertificate 设置的证书
       */
        void SetNetAgentQuicECCRootCertificate(std::shared_ptr<CZegoCACertData> cert_data);

         /**
         设置0 rtt 相关数据 
        
        @param verifyInfo 缓存的验证数据
        @param is_ecc 是否是ecc 证书
        ...
        */
        ...
        @param info 调度配置相关信息
        @discussion 必须调用此API来触发拉取或更新Dispatch，如有启用0RTT 功能。则必须在调用SetNetAgent0RttVerifyInfo 接口后，才能调用SetDispatchResult接口
        */
        void SetDispatchResult(ZegoNetAgentAddressInfo* pResult);
		...
		*/
        ...
        /**
         ...
         @param delegate 回调处理函数
         */
        void SetReportNetAgentProxyServicesConnectEventDelegate(const OnReportProxyServicesConnectEventDelegate& delegate);
