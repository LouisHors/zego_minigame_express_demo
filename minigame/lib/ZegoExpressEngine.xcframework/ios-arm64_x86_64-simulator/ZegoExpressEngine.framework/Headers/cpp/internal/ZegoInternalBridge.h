...
class ZegoExpressEngineBridge {
  ...
  public:
    ...
    int addPublishCdnUrl(const char *streamID, const char *targetURL) {
        int seq = 0;
        zego_express_add_publish_cdn_url(streamID, targetURL, &seq);
        return seq;
    }
    ...
    int copyrightedMusicGetCurrentPitch(const char *resource_id) {
        int score = 0;
        int ret = zego_express_copyrighted_music_get_current_pitch(resource_id, &score);
        return score;
    }
    ...
    int setReciveRange(int range_scene_handle, float range) {
